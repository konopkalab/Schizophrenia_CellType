###############################################################################################
#Steps to run DSS to identify differentially methylated CpGs in sz vs control for each cell-type
###############################################################################################
library(bsseq)
library(DSS)


#read bsseq object with all individual and all CpGs per cell-type
...
#col3:Diagnosis
#col4:Cell_type (not necessary in this analysis)
#col5:Sex
...
#3-put PCs in same order as covariates
match2<-match(data4n[,13],rownames(data3))
data3n<-data3[match2,]


#4-run DSS for diagnosis using covariates + 10 genetic PCs in the model
design<-data.frame(as.factor(data4n[,3]),as.factor(data4n[,5]),as.factor(data4n[,7]),as.factor(data4n[,8])
                  ,as.factor(data4n[,10]),as.factor(data4n[,11]),as.numeric(data4n[,12]),data3n)
colnames(design)<-c("disease","sex","age","bank","pmi","hemi","conv",paste0("pc",1:10))
DMLfit = DMLfit.multiFactor(data1, design, ~disease+sex+age+bank+pmi+hemi+conv+pc1+pc2+pc3+pc4+pc5+pc6+pc7+pc8+pc9+pc10)
res = DMLtest.multiFactor(DMLfit, coef=2)
save.image(paste("DSS_sz_10geneticPCs.Rds",sep=''))
write.csv(res,paste0("DSS_sz_10geneticPCs.csv"),quote=F,row.names=F)
...
for(i in 1:dim(meth.df)[1]) {
	try(cur.res <- lm(as.numeric(meth.df[i,])~design$disease+design$sex+design$age+design$bank+design$pmi+design$hemi+design$conv+design$pc1+design$pc2+design$pc3+design$pc4+design$pc5+design$pc6+design$pc7+design$pc8+design$pc9+design$pc10))
...
write.csv(lm.res,paste0("DSS_sz_10geneticPCs.csv"),quote=F,row.names=F)
