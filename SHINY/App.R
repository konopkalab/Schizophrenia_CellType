...
server <- function(input, output) {
  ...
  output$vioPlot<-renderPlot({
    ...
    tocompare <- list( c("SCZ", "CTL"))
    vPlot=ggviolin(plot_data, x = "Category", y = "Expression", color = "Category",
      ...
      stat_compare_means(method = "anova",label.y = max(plot_data$Expression)*1.2,size = 6)+
      xlab("")+
  ...
  observe({
    ...
    tocompare <- list( c("SCZ", "CTL"))
      ...
      color = "Category", palette =c("#56B4E9","#E69F00"),
...
ui <- fluidPage(
  ...
  # Application title
  titlePanel(h1("Schizophrenia CellType DGE")),
