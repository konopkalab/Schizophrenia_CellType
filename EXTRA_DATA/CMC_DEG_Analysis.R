...
#' Data was downloaded from Synapse portal.
...
#load the data
#exp <- read.table("CMC_MSSM-Penn-Pitt_DLPFC_mRNA_IlluminaHiSeq2500_exonExpressionRaw.tsv",sep="\t")
#exp$ID <- gsub("\\:..*","",rownames(exp))
...
#agg$ID <- NULL
#exp <- agg
#TRAITS_CMC <- read.table("CMC_Demo.txt")
#save(exp,TRAITS_CMC,file="CMC_INPUT.RData")
...
#' Filter the data
load("INPUT_DATA/CMC_INPUT.RData")
rownames(TRAITS_CMC) <- TRAITS_CMC$Sample_ID
TRAITS_CMC$ID <- NULL
TRAITS_CMC$Sample_ID <- NULL
exp <- exp[,colnames(exp)%in%rownames(TRAITS_CMC)]
exp <- exp[,match(rownames(TRAITS_CMC),colnames(exp))]
dat <- log2(cpm(exp)+1)
class <- unique(TRAITS_CMC$Diagnosis)
...
pdf("OUTPUT_CMC/Variance_Explained_CMC.pdf",width=8,height=6)
...
form <- ~ (1|Diagnosis) + (1|Sex) + (1|BrainBank) + RIN + (1|Age) + (1|Pmi)
...
#' Create model matrix
mod <- model.matrix(~Diagnosis+Sex+BrainBank+Age+Pmi+RIN, data =pd)
mod0 <- model.matrix(~Sex+BrainBank+Age+Pmi+RIN, data = pd)
...
fitEb = eBayes(fitLM);
CMC_DGE = topTable(fitEb, coef = "DiagnosisSchizo",number=nrow(p));
write.table(CMC_DGE, file = paste(output_folder,suffix, "_LIMMA_DEG", ".txt",sep = ""),sep="\t",quote=F)

#' DGE table
DT::datatable(CMC_DGE, options = list(pageLength = 10))

#' Save into a .RData
CMC_DATA=list(CMC_ExpQuant=p,CMC_ExpCleaned_lm=pAdj,CMC_pData=pdSVs,CMC_Dge_All=CMC_DGE)
save(CMC_DATA,file="OUTPUT_CMC/CMC_EXON_DATA.RData")
