...
suffix <- strsplit(colnames(log2CPM_NEUN)[1],"_")[[1]][[3]]
...
data <- log2CPM_NEUN[!(rownames(log2CPM_NEUN) %in% unique(filtBed$V4)),]
class <- unique(TRAITS_NEUN[,1])
...
fitEb = eBayes(fitLM);
NeuN_DGE = topTable(fitEb, coef = "DiagnosisSchizo",number=nrow(p));
write.table(NeuN_DGE, file = paste(output_folder,suffix, "_EXON_LIMMA_DEG", ".txt",sep = ""),sep="\t",quote=F)

#' DGE table
DT::datatable(NeuN_DGE, options = list(pageLength = 10))

#' Save into a .RData
NEUN_EXON_DATA=list(NeuN_ExpQuant=p,NeuN_ExpCleaned_lm=pAdj,NeuN_pData=pdSVs,NeuN_Dge_All=NeuN_DGE)
save(NEUN_EXON_DATA,file="OUTPUT_NeuN/NEUN_EXON_DATA.RData")

#' To run the CrossValidation please do: R CMD BATCH --vanilla CrossValidation_NeuN.R
